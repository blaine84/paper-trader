...
# table) and the run() result shape they assert on (force_closes /
# hard_wall_closes / alerts) no longer exist in agents.position_timer and have
# no equivalent. Skipped rather than deleted to preserve the original test
# intent for reference.
pytestmark = pytest.mark.skip(
    reason="Targets pre-refactor position_timer API (_alert_status, "
    "_revalidate_momentum_fade, SETUP_TIME_LIMITS, force_closes/hard_wall_closes/"
    "alerts result keys) removed in commit 09d7b10 when policy moved to "
    "utils.position_lifecycle_governance."
)
