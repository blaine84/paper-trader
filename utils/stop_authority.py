...
# Cooldown between accepted maintenance stop updates (seconds)
MAINTENANCE_STOP_COOLDOWN_SECONDS = 900  # 15 minutes
...
def _is_maintenance_source(source_agent: str, stop_role: str) -> bool:
    """Return True if this stop update is subject to maintenance discipline rules.

    Included roles:
      - "maintenance_tighten": routine PM maintenance tightening
      - "trail": profit_manager trailing stop adjustments
      - "breakeven": breakeven moves generated by profit/maintenance paths

    Included agents:
      - "portfolio_manager"
      - "profit_manager"

    Excluded (bypass maintenance rules):
      - stop_role="initial" (trade entry)
      - Any source_agent not in the maintenance set (e.g., price_monitor)
      - Any stop_role not in the maintenance set (e.g., "exit", "emergency")
    """
    MAINTENANCE_ROLES = {"maintenance_tighten", "trail", "breakeven"}
    MAINTENANCE_AGENTS = {"portfolio_manager", "profit_manager"}
    return stop_role in MAINTENANCE_ROLES and source_agent in MAINTENANCE_AGENTS


def _check_monotonic(side: str, old_stop: float, new_stop: float) -> bool:
    """Return True if the proposed stop is strictly tighter (reduces risk).

    Long: new_stop > old_stop (moves up toward price)
    Short: new_stop < old_stop (moves down toward price)
    Equal values return False (treated as no-op).
    """
    if side == "long":
...
def _check_minimum_change(
    old_stop: float,
    new_stop: float,
    current_price: float | None,
    atr: float | None,
) -> bool:
    """Return True if the stop change exceeds the minimum threshold.

    Threshold logic:
      - If current_price and atr are both available:
            threshold = max(0.0025 * current_price, 0.25 * atr)
      - If atr is None but current_price is available:
            threshold = 0.0025 * current_price
      - If current_price is None (regardless of atr):
            threshold = 0.001 * old_stop

    Returns True when abs(new_stop - old_stop) >= threshold.
    """
    if current_price is not None:
        price_threshold = 0.0025 * current_price
        if atr is not None:
            threshold = max(price_threshold, 0.25 * atr)
        else:
            threshold = price_threshold
    else:
        threshold = 0.001 * old_stop

    return abs(new_stop - old_stop) >= threshold


def _check_cooldown(
    db, trade_id: int, cooldown_seconds: int = 900
) -> bool:
    """Return True if cooldown has expired (update is allowed).

    Queries the latest stop_update_accepted event for this trade_id.
    If no prior accepted event exists, returns True (no cooldown applies).
    On DB error: logs warning and returns True (fail open).
    """
    try:
        latest_event = (
            db.query(TradeEvent)
            .filter(
                TradeEvent.trade_id == trade_id,
                TradeEvent.event_type == "stop_update_accepted",
            )
        ...
        log.warning(
            "Cooldown check DB error for trade_id=%s, failing open: %s",
            trade_id,
            exc,
        )
        return True
    ...
    elapsed = (datetime.utcnow() - latest_event.timestamp).total_seconds()
...
def apply_stop_update(
    ...
    *,
    ...
    # Step 1.5: Maintenance discipline checks (between log and geometry validation)
    if old_stop is not None and _is_maintenance_source(source_agent, stop_role):
        # Monotonic check (cheapest — pure arithmetic)
        if not _check_monotonic(side, old_stop, new_stop):
            _log_stop_event(
                db,
                ...
                profile=profile,
                price=new_stop,
                message="non-monotonic maintenance stop",
                payload={
                    "source_agent": source_agent,
                    "proposed_stop": new_stop,
                    "existing_stop": old_stop,
                    "rejection_reason": "non-monotonic maintenance stop",
                    "stop_role": stop_role,
                    "side": side,
                },
            )
            return StopValidationResult(
                valid=False,
                reason_type="rejected",
                reason="non-monotonic maintenance stop",
                old_stop=old_stop,
                new_stop=new_stop,
                side=side,
                stop_role=stop_role,
        ...
        # Minimum change check (pure arithmetic)
        if not _check_minimum_change(old_stop, new_stop, current_price, atr):
            _log_stop_event(
                db,
                "stop_update_rejected",
                trade_id=trade_id,
                ...
                profile=profile,
                price=new_stop,
                message="below minimum stop change threshold",
                payload={
                    "source_agent": source_agent,
                    "proposed_stop": new_stop,
                    "existing_stop": old_stop,
                    "rejection_reason": "below minimum stop change threshold",
                    "stop_role": stop_role,
                    "side": side,
                },
            )
            return StopValidationResult(
                valid=False,
                reason_type="rejected",
                reason="below minimum stop change threshold",
                old_stop=old_stop,
                new_stop=new_stop,
                side=side,
                stop_role=stop_role,
        ...
        # Cooldown check (DB query — most expensive, runs last)
        if not _check_cooldown(db, trade_id, MAINTENANCE_STOP_COOLDOWN_SECONDS):
            _log_stop_event(
                db,
                "stop_update_rejected",
                trade_id=trade_id,
                ...
                profile=profile,
                price=new_stop,
                message="stop update cooldown active",
                payload={
                    "source_agent": source_agent,
                    "proposed_stop": new_stop,
                    "existing_stop": old_stop,
                    "rejection_reason": "stop update cooldown active",
                    "stop_role": stop_role,
                    "side": side,
                },
            )
            return StopValidationResult(
                valid=False,
                reason_type="rejected",
                reason="stop update cooldown active",
                old_stop=old_stop,
                new_stop=new_stop,
                side=side,
                stop_role=stop_role,
